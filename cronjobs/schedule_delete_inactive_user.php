...
class ScheduleDeleteInactiveUser extends CronJob
{
    ...
    private static function scheduleForDeleteAndInform($status_info, $remind = false){
        if (self::sendInfoMail($status_info->user_id, $remind)){
            ...
            $sec_per_day = 86400;
            if (!$remind){
                //L�schung in x tagen, siehe konfiguration
                ...
                $status_info->store();
                //wenn zur L�schung vorgemerkt (status == 1)
    ...
    public function execute($last_result, $parameters = array())
    {
        ...
        if (Config::get()->getValue(USERMANAGEMENT_TEST_MODE)){
            ...
            echo 'los gehts';
            //wenn letzte Nutzeraktivit�t l�nger her ist als x (x wird in Konfiguration festgelegt)
            $db = DBManager::get();
    ...
    private function check_on_user($user_id){
        ...
        if ($user->Email == 'rlucke@uos.de'){
            ...
            //wenn die G�ltigkeit des Accounts von der Aktivit�t abh�ngt und noch keine Mail versendet wurde (status == 0)
            if ($status_info->account_status == 0 && $status_info->delete_mode == 'aktivitaet'){
                ...
                $time = time();
                //wenn Halbzeit bis L�schung: Erinnerungsmail //TODO 2 fr�hestens wochen vor L�schung
            ...
            //Nutzer hat alle Infomails erhalten (2) oder fehlerhafte Mailadresse soll ignoriert werden (5)   
                ...
                //account nicht mehr g�ltig und zur L�schung vorgesehen
                if (UserConfig::get($user_id)->getValue(EXPIRATION_DATE) < time()){
                    ...
                    foreach($seminare_dozent as $membership){
                        ...
                        //falls kein Seminar existiert in welchem dieser Nutzer einziger Dozent ist: Account l�schen
                    } if (!$single_dozent_in_seminar){
                        $user_mng = new UserManagement($user_id);
                        $user_mng->deleteUser(false); //false: Dokumente nicht l�schen
                        //account gel�scht (status == 6)
                        $status_info->account_status = 6;
                        $status_info->chdate = time();
                        $status_info->store();
                    } else {
                        //Dozent kann nicht gel�scht werden weil einziger Dozent in VA (status == 4)
